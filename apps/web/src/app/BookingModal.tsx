...
export default function BookingModal({ checkIn, checkOut, onClose }: BookingModalProps) {
  ...
  const [parkingType, setParkingType] = useState<"none" | "car" | "motorcycle">("none");
      ...
      {state.status === "success" ? <section className="booking-modal-success" role="status">
        ...
        <div className="booking-modal-scroll">
          ...
          <form action={action} className="booking-modal-form booking-stitch-form">
            ...
            <input type="hidden" name="idempotencyKey" value={idempotencyKey}/><input type="hidden" name="roomTypeId" value=""/><input type="hidden" name="preferredContact" value="phone"/><input type="hidden" name="bedroomChoice" value={bedroomChoice}/><input type="hidden" name="parkingType" value={parkingType}/>
            <label className="booking-honeypot">Website<input name="website" tabIndex={-1} autoComplete="off"/></label>
            <section className="booking-suite-section"><span>CHOOSE YOUR STAY</span><div className="booking-room-options">{([ ["both_bedrooms", "Entire two-bedroom condo", "Queen master · Double-size bunk room · Kitchen", "₱2,200 / night"], ["bedroom_1", "Master bedroom", "Queen bed · Shared kitchen and bathroom", "₱1,700 / night"], ["bedroom_2", "Second bedroom", "Double-size bunk bed · Shared kitchen and bathroom", "₱1,700 / night"] ] as const).map(([value, title, detail, price]) => <button type="button" className={bedroomChoice === value ? "selected" : undefined} onClick={() => setBedroomChoice(value)} key={value}><span><strong>{title}</strong><small>{detail}</small></span><b>{price}</b></button>)}</div></section>
            ...
            <section className="booking-parking-section"><div className="booking-section-label"><span>PARKING</span><small>Optional, subject to availability</small></div><div className="booking-parking-options">{([["none","No parking","₱0 / night"],["car","Car parking","₱350 / night"],["motorcycle","Motorcycle parking","₱150 / night"]] as const).map(([value,label,price]) => <button type="button" className={parkingType === value ? "selected" : undefined} onClick={() => setParkingType(value)} key={value}><strong>{label}</strong><b>{price}</b></button>)}</div></section><div className="booking-preview-photos"><figure><Image src="/images/uppadar-hollie/master-bedroom.jpg" alt="Master bedroom" fill sizes="220px"/><figcaption>Master bedroom</figcaption></figure><figure><Image src="/images/uppadar-hollie/dining-table.png" alt="Living and dining area" fill sizes="220px"/><figcaption>Living &amp; dining</figcaption></figure></div>
